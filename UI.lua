...
local aa = {
	...
	function()
		...
		return function(m)
			...
			n.AddParent = function(r)
				h.AddSignal(r:GetPropertyChangedSignal"Visible", function() n.SetVisibility(r.Visible) end)
			end
	...
	function()
		...
		return function(k)
			...
			l.Frame = j("Frame", {
				...
				j("Frame", { BackgroundColor3 = Color3.fromRGB(255, 255, 255), BackgroundTransparency = 0.4, Size = UDim2.fromScale(1, 1) }, {
					j("UICorner", { CornerRadius = UDim.new(0, 8) }),
	...
	function()
		local c, d, e, f, g = b(6)
		local h, i = function(h, i, j, k, l) return (h - i) * (l - k) / (j - i) + k end, function(h, i)
	...
	[9] = function()
		...
		return function(m, n, o)
			...
			p.Frame = k("TextButton", { Size = UDim2.new(0, 0, 0, 32), Parent = n, ThemeTag = { BackgroundColor3 = "DialogButton" } }, {
				...
				k("UIStroke", { ApplyStrokeMode = Enum.ApplyStrokeMode.Border, Transparency = 0.65, ThemeTag = { Color = "DialogButtonBorder" } }),
				p.HoverFrame,
				p.Title
			})
	...
	[10] = function()
		...
		function q.Create(r)
			...
			s.ButtonHolderFrame = p("Frame", { Size = UDim2.new(1, 0, 0, 70), Position = UDim2.new(0, 0, 1, -70), ThemeTag = { BackgroundColor3 = "DialogHolder" } }, {
	...
	[11] = function()
		...
		return function(m, n, o, p)
			...
			q.LabelHolder = k("Frame", {
				...
				k("UIListLayout", { SortOrder = Enum.SortOrder.LayoutOrder, VerticalAlignment = Enum.VerticalAlignment.Center }),
			...
			function q.SetDesc(r, s)
				if s == nil then s = "" end
				if s == "" then q.DescLabel.Visible = false
				else q.DescLabel.Visible = true end
				q.DescLabel.Text = s
			end
	...
	[12] = function()
		...
		function o.New(p, q)
			...
			r.LabelHolder = n("Frame", {
				...
				n("UIListLayout", { SortOrder = Enum.SortOrder.LayoutOrder, VerticalAlignment = Enum.VerticalAlignment.Center, Padding = UDim.new(0, 3) }),
				r.ContentLabel,
				r.SubContentLabel
			})
			...
			local p = function()
				...
				if not o.Input:IsFocused() or o.Input.TextBounds.X <= q - 2 * p then o.Input.Position = UDim2.new(0, p, 0, 0)
					...
					if r ~= -1 then
						...
						if u < p then o.Input.Position = UDim2.fromOffset(p - t, 0)
						elseif u > q - p - 1 then o.Input.Position = UDim2.fromOffset(q - t - p - 1, 0) end
					end
				end
			end
			task.spawn(p)
		...
		return function(n)
			...
			o.Frame = l("Frame", { Size = UDim2.new(1, 0, 0, 42), BackgroundTransparency = 1, Parent = n.Parent }, {
				l("Frame", { Size = UDim2.new(1, -16, 1, 0), Position = UDim2.new(0, 16, 0, 0), BackgroundTransparency = 1 }, {
					l("UIListLayout", { Padding = UDim.new(0, 5), FillDirection = Enum.FillDirection.Horizontal, SortOrder = Enum.SortOrder.LayoutOrder }),
					l("TextLabel", {
						RichText = true,
						Text = n.Title,
						FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Regular, Enum.FontStyle.Normal),
	...
	[17] = function()
		...
		return function(t)
			...
			v.Maximize = function(M, N, O)
				...
				G:setGoal{ X = l[O and "Instant" or "Spring"].new(P, { frequency = 6 }), Y = l[O and "Instant" or "Spring"].new(Q, { frequency = 6 }) }
			...
			m.AddSignal(v.TitleBar.Frame.InputBegan, function(M)
				if M.UserInputType == Enum.UserInputType.MouseButton1 or M.UserInputType == Enum.UserInputType.Touch then
					...
					M.Changed:Connect(function()
						if M.UserInputState == Enum.UserInputState.End then w = false end
					end)
				end
			end)
			m.AddSignal(v.TitleBar.Frame.InputChanged, function(M)
				if M.UserInputType == Enum.UserInputType.MouseMovement or M.UserInputType == Enum.UserInputType.Touch then x = M end
			end)
			...
			m.AddSignal(h.InputChanged, function(M)
				...
				if (M.UserInputType == Enum.UserInputType.MouseMovement or M.UserInputType == Enum.UserInputType.Touch) and A then
					...
					local Q = Vector2.new(math.clamp(P.X, 470, 2.048e3), math.clamp(P.Y, 380, 2.048e3))
			...
			m.AddSignal(h.InputBegan, function(M)
				if type(u.MinimizeKeybind) == "table" and u.MinimizeKeybind.Type == "Keybind" and not h:GetFocusedTextBox() then
					if M.KeyCode.Name == u.MinimizeKeybind.Value then v:Minimize() end
	...
	[18] = function()
		...
		function k.UpdateTheme()
			for m, n in next, k.Registry do
				for o, p in next, n.Properties do m[o] = k.GetThemeProperty(p) end
			end
		...
		function k.SpringMotor(m, n, o, p, s)
			...
			local u = function(u, v)
				...
				if not p then
					if not v then
						if o == "BackgroundTransparency" and e(h).DialogOpen then return end
					end
				end
		...
		function u.New(v, w, x)
			...
			local aa, ab = s("ImageLabel", {
				...
				local N, O = s("ImageLabel", {
					...
					Parent = C.Root
				}, { s("UICorner", { CornerRadius = UDim.new(0, 4) }), s("UIStroke", { Thickness = 2, Transparency = 0.75 }), M }), s("Frame", { BackgroundColor3 = z.Value, Size = UDim2.fromScale(1, 1), BackgroundTransparency = 0 }, { s("UICorner", { CornerRadius = UDim.new(0, 4) }) })
				local P, Q = s("ImageLabel", {
					...
					Size = UDim2.fromOffset(88, 24),
					Parent = C.Root
				}, { s("UICorner", { CornerRadius = UDim.new(0, 4) }), s("UIStroke", { Thickness = 2, Transparency = 0.75 }), O }), {}
				for R = 0, 1, 0.1 do
				...
				p.AddSignal(W.Input.FocusLost, function(ad)
					if ad then
						...
						if af and typeof(ag) == "Color3" then
							if tonumber(W.Input.Text) <= 255 then D, E, F = Color3.toHSV(ag) end
						end
					end
					ac()
				end)
				p.AddSignal(X.Input.FocusLost, function(ad)
					if ad then
						...
						if af and typeof(ag) == "Color3" then
							if tonumber(X.Input.Text) <= 255 then D, E, F = Color3.toHSV(ag) end
						end
					end
					ac()
				end)
				p.AddSignal(Y.Input.FocusLost, function(ad)
					if ad then
						...
						if af and typeof(ag) == "Color3" then
							if tonumber(Y.Input.Text) <= 255 then D, E, F = Color3.toHSV(ag) end
						end
					end
					ac()
				end)
	...
	[22] = function()
		...
		function g.New(h, i, j)
			...
			local y, z = function()
				if #l.Values > 10 then v.Size = UDim2.fromOffset(x, 392)
				else v.Size = UDim2.fromOffset(x, s.AbsoluteContentSize.Y + 10) end
			end, function() t.CanvasSize = UDim2.fromOffset(0, s.AbsoluteContentSize.Y) end
			w()
			y()
			...
			function l.GetActiveValues(B)
				if j.Multi then
					...
					return C
				else return l.Value and 1 or 0 end
			end
			...
			function l.BuildDropdownList(B)
				...
				for J, K in next, D do
					if J.ButtonLabel then
						if J.ButtonLabel.TextBounds.X > x then x = J.ButtonLabel.TextBounds.X end
					end
				end
				x = x + 30
				z()
			...
			function l.SetValue(B, C)
				if l.Multi then
					...
					if not C then l.Value = nil
					elseif table.find(l.Values, C) then l.Value = C end
				end
				l:BuildDropdownList()
				k:SafeCallback(l.Callback, l.Value)
				k:SafeCallback(l.Changed, l.Value)
			...
			if next(B) then
				for C = 1, #B do
					local D = B[C]
					if j.Multi then l.Value[l.Values[D]] = true
	...
	[23] = function()
		...
		function c.New(d, e, f)
			...
			if h.Finished then
				...
				ai(k:GetPropertyChangedSignal"Text", function() h:SetValue(k.Text) end)
			end
	...
	[24] = function()
		...
		function c.New(d, e, f)
			...
			local l = ai("TextButton", {
				...
				ai("UIStroke", { Transparency = 0.5, ApplyStrokeMode = Enum.ApplyStrokeMode.Border, ThemeTag = { Color = "InElementBorder" } }),
				k
			})

			function h.GetState(m)
				if af:GetFocusedTextBox() and h.Mode ~= "Always" then return false end
				if h.Mode == "Always" then return true
			...
			ah.AddSignal(l.InputBegan, function(m)
				if m.UserInputType == Enum.UserInputType.MouseButton1 or m.UserInputType == Enum.UserInputType.Touch then
					...
					n = af.InputBegan:Connect(function(o)
						local p
						if o.UserInputType == Enum.UserInputType.Keyboard then p = o.KeyCode.Name
						elseif o.UserInputType == Enum.UserInputType.MouseButton1 then p = "MouseLeft"
						elseif o.UserInputType == Enum.UserInputType.MouseButton2 then p = "MouseRight" end
						local s
	...
	[26] = function()
		...
		function c.New(d, e, f)
			...
			local l, m, n = ai("Frame", { BackgroundTransparency = 1, Position = UDim2.fromOffset(7, 0), Size = UDim2.new(1, -14, 1, 0) }, { k }), ai("Frame", { Size = UDim2.new(0, 0, 1, 0), ThemeTag = { BackgroundColor3 = "Accent" } }, { ai("UICorner", { CornerRadius = UDim.new(1, 0) }) }), ai("TextLabel", {
				FontFace = Font.new"rbxasset://fonts/families/GothamSSm.json",
				Text = "Value",
				TextSize = 12,
				TextWrapped = true,
			...
			ah.AddSignal(k.InputBegan, function(p)
				if p.UserInputType == Enum.UserInputType.MouseButton1 or p.UserInputType == Enum.UserInputType.Touch then i = true end
			end)
			ah.AddSignal(k.InputEnded, function(p)
				if p.UserInputType == Enum.UserInputType.MouseButton1 or p.UserInputType == Enum.UserInputType.Touch then i = false end
			end)
	...
	[31] = function()
		...
		function ai.start(aj)
			if not aj._connection then
				aj._connection = af.RenderStepped:Connect(function(c) aj:step(c) end)
			end
		end
	...
	[33] = function()
		...
		local aj = function(aj)
			...
			local c = typeof(aj)
			if c == "number" then return ag.new(aj, false)
			elseif c == "table" then return ai.new(aj, false) end
		...
		function ai.new(c, d)
			...
			if d ~= nil then e._useImplicitConnections = d
			else e._useImplicitConnections = true end
			e._complete = true
			e._motors = {}
	...
	[34] = function()
		...
		return function()
			...
			it("should complete when all child motors are complete", function()
				...
				ai:step(0.0166666667)
				expect(ai._complete).to.equal(false)
				for aj = 1, 30 do ai:step(0.0166666667) end
				expect(ai._complete).to.equal(true)
			end)
	...
	[38] = function()
		...
		return function()
			...
			describe("completed state", function()
				...
				ah:setGoal(ai)
				for aj = 1, 60 do ah:step(0.0166666667) end
			...
			describe("uncompleted state", function()
				local ah, ai = af.new(0, false), ag.new(1, { velocity = 1 })
				ah:setGoal(ai)
				for aj = 1, 59 do ah:step(0.0166666667) end
	...
	[41] = function()
		...
		function ag.new(ah, aj)
			...
			if aj ~= nil then c._useImplicitConnections = aj
			else c._useImplicitConnections = true end
			c._goal = nil
	...
	[43] = function()
		...
		local af, ag, ah, aj = 1e-3, 1e-3, 1e-4, {}
		aj.__index = aj
	...
	[44] = function()
		...
		return function()
			...
			describe("completed state", function()
				...
				ah:setGoal(aj)
				for c = 1, 100 do ah:step(0.0166666667) end
				it("should complete", function() expect(ah._state.complete).to.equal(true) end)
				it("should be exactly the goal value when completed", function() expect(ah._state.value).to.equal(1) end)
			end)
	...
	return {
		Name = "Blue",
		Accent = Color3.fromRGB(0, 148, 255), -- ใช้สีฟ้าที่ต้องการเป็น accent
		AcrylicMain = Color3.fromRGB(60, 60, 60),
		AcrylicBorder = Color3.fromRGB(90, 90, 90),
		AcrylicGradient = ColorSequence.new(Color3.fromRGB(40, 40, 40), Color3.fromRGB(40, 40, 40)),
		AcrylicNoise = 0.9,
		TitleBarLine = Color3.fromRGB(75, 75, 75),
		Tab = Color3.fromRGB(100, 140, 160), -- โทนที่ใกล้เคียงฟ้า
		Element = Color3.fromRGB(100, 140, 160),
		ElementBorder = Color3.fromRGB(35, 35, 35),
		InElementBorder = Color3.fromRGB(90, 90, 90),
		ElementTransparency = 0.87,
		ToggleSlider = Color3.fromRGB(100, 140, 160),
		ToggleToggled = Color3.fromRGB(0, 30, 60), -- โทนดำที่มีฟ้านิดๆ
		SliderRail = Color3.fromRGB(100, 140, 160),
		DropdownFrame = Color3.fromRGB(0, 148, 255), -- ใช้สีฟ้าที่ต้องการ
		DropdownHolder = Color3.fromRGB(45, 45, 45),
		DropdownBorder = Color3.fromRGB(35, 35, 35),
		DropdownOption = Color3.fromRGB(100, 140, 160),
		Keybind = Color3.fromRGB(100, 140, 160),
		Input = Color3.fromRGB(0, 148, 255), -- ใช้สีฟ้าที่ต้องการ
		InputFocused = Color3.fromRGB(10, 10, 10),
		InputIndicator = Color3.fromRGB(0, 148, 255), -- ใช้สีฟ้าที่ต้องการ
		...
		DialogInputLine = Color3.fromRGB(0, 148, 255), -- ใช้สีฟ้าที่ต้องการ
		Text = Color3.fromRGB(240, 240, 240),
		SubText = Color3.fromRGB(170, 170, 170),
		Hover = Color3.fromRGB(100, 140, 160), -- เพิ่มฟ้านิดๆเมื่อ hover
		HoverChange = 0.07
	}
end,
	[51] = function()
...
do
	...
	local l, m, n, o, p, s, t, u, v, w, x = ah.insert, ah.remove, ah.freeze or function(l) return l end, ab and ab.defer or function(l, ...)
		local m = c.create(l)
		c.resume(m, ...)
		return m
	end, "0.0.0-venv", {}, {}, {}, {}, {}, {}
